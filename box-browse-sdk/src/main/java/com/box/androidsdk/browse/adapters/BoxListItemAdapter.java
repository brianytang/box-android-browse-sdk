...
public class BoxListItemAdapter extends ArrayAdapter<BoxListItem> {
    ...
    public synchronized View getView(final int position, final View convertView, final ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            if (this.getItemViewType(position) == BoxListItem.TYPE_BOX_FOLDER_ITEM) {
                row = LayoutInflater.from(getContext()).inflate(R.layout.boxsdk_box_folder_list_item, parent, false);
            }
            else {
    ...
    protected void update(final ViewHolder holder, final BoxListItem listItem) {
        if (listItem.getType() == BoxListItem.TYPE_FUTURE_TASK) {
            if (!listItem.getTask().isDone()) {
                ...
                holder.getNameView().setText(getContext().getResources().getString(R.string.boxsdk_Please_wait));
            }
            return;
        }
    ...
    public void set(BoxListItems collection) {
        this.clear();
        this.add(collection);

    }

    public void add(final BoxListItems collection) {
        setNotifyOnChange(false);

        Iterator<BoxItem> it = collection.iterator();
        while (it.hasNext()) {
            BoxItem item = it.next();
            if (item instanceof BoxFile) {
                add(new BoxListItem((BoxFile) item, item.getId()));
            }
            else if (item instanceof BoxFolder) {
                add(new BoxListItem((BoxFolder) item, item.getId()));
            }
        }
